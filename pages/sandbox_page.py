...
#el objetivo de este documento es automatizar la seleccion de un producto en la pagina
class SandboxPage(BasePage): #clase base para crear paginas de selenium
    ENVIAR_BUTTON = (By.XPATH, "//button[contains(text(), 'Enviar')]")
    DYNAMIC_ID_BUTTON = ( #elemento de id dinamico
        By.XPATH,
    ...
    def select_checkbox_with_label(self, label_text): # es un metodo para seleccionar uno o varios checkbox
        ...
        checkbox_locator = (
            By.XPATH,
            f"//label[contains(., '{label_text}')]/preceding-sibling::input[@type='checkbox']", #elegime el checkbox para esta opcion
        )
    ...
    def select_deporte(self, deporte):
        self.select_by_visible_text(self.DEPORTE_DROPDOWN, deporte)

    
    def get_deporte_dropdown_options(self):
        return self.get_options(self.DEPORTE_DROPDOWN)
